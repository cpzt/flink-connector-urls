 ...
 */
public abstract class URLSourceReaderBase<E extends IRecord<T>, T,SplitT extends SourceSplit, SplitStateT>
        implements SourceReader<T, SplitT> {
    private static final Logger LOG = LoggerFactory.getLogger(URLSourceReaderBase.class);
    ...
    @Nullable private Function<T, SplitT[]> addSplitFunction;

    /** Indicating whether the SourceReader will be assigned more splits or not. */
    private boolean noMoreSplitsAssignment;

    public URLSourceReaderBase(
            FutureCompletingBlockingQueue<RecordsWithSplitIds<E>> elementsQueue,
            SplitFetcherManager<E, SplitT> splitFetcherManager,
            Function<T, SplitT[]> addSplitFunction,
            RecordEmitter<E, T, SplitStateT> recordEmitter,
            Configuration config,
            SourceReaderContext context) {
        this.elementsQueue = elementsQueue;
        this.splitFetcherManager = splitFetcherManager;
        this.addSplitFunction = addSplitFunction;
    ...
    public InputStatus pollNext(ReaderOutput<T> output) throws Exception {
        ...
        while (true) {
            ...
            if (record != null) {
                ...
                if (addSplitFunction != null) {
                    T data = record.getRecord();
                    if (null != data) {
                        sendSourceEventToEnumerator(addSplitFunction.apply(data));
                    }
                }
    ...
    public void sendSourceEventToEnumerator(SplitT[] splits) {}
}
